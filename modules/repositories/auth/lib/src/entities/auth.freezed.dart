...
class _$AuthTearOff {
  ...
  _Auth call({required User? user, required String? token}) {
    return _Auth(
      user: user,
      token: token,
    );
...
class _$_Auth with DiagnosticableTreeMixin implements _Auth {
  const _$_Auth({required this.user, required this.token});
...
abstract class _Auth implements Auth {
  const factory _Auth({required User? user, required String? token}) = _$_Auth;
